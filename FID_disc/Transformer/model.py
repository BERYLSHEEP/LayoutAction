...
class FID_GPTnet(nn.Module):
    ...
    def forward(self, idx, targets=None, pad_token=-100):
        b, t = idx.size()
        feats = self.collect_features(idx)
        logit_disc = self.fc_out_disc(feats[:, 0, :]).squeeze(-1)

        if self.config.is_enc_dec:
            x = feats[:, 0, :].unsqueeze(1).expand(-1, t, -1)
        ...
        return logit_disc, logit_pred, loss, feats[:, 0, :]