...
class FlatDirectoryNumpyDataset(Dataset):
    """ pyTorch Dataset wrapper for the generic flat directory images dataset """
    ...
    def __getitem__(self, idx):
        """
        obtain the numpy file (read and transform)
        :param idx: index of the file required
        :return: numpy array
        """
        np_file = self.files[idx]

        if np_file[-4:] == ".npy":
            # files are in .npy format
            numpy = np.load(np_file)

        # apply the transforms on the numpy file
        if self.transform is not None:
            numpy = self.transform(numpy)
