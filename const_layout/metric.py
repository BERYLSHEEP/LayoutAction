...
from models.layoutnet import LayoutNet
from util import convert_xywh_to_ltrb
from datas.util import RelSize, RelLoc, detect_size_relation, detect_loc_relation
...
class LayoutFID():
    ...
    #     # load pre-trained LayoutNet
    #     # tmpl = './output/rico/FIDNet/20220117181833731330/checkpoint.pth.tar'
    #     # # tmpl = './pretrained/layoutnet_{}.pth.tar'
    #     # state_dict = torch.load(tmpl.format(dataset_name), map_location=device)
    ...
    # def collect_features(self, model, bbox, label, padding_mask, real=False):
    #     if real and type(self.real_features) != list:
    #         return

    #     feats = model.extract_features(bbox.detach(), label, padding_mask)
    #     features = self.real_features if real else self.fake_features
    #     features.append(feats.cpu().numpy())
...
def __compute_maximum_iou_for_layout(layout_1, layout_2):
    ...
    for l in list(set(li.tolist())):
        ...
        '''
        each Matric C[i, j] is the cost fo matching vertex i of the first partite set(a "worker") and vertex j
        of the second set(a "job"). The goal is to find a complete assignment of workers to jobs of maximum cost
        '''
